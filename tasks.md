...
- Extend shuffle functionality to support filtered slideshows. Implemented new-shuffle, tagged-shuffle, and untagged-shuffle sequence modes that combine photo filtering with shuffle ordering. Users can now choose between sequential or shuffled viewing for each slideshow type (new, tagged, untagged) via dashboard buttons. All shuffle modes maintain consistent navigation with session-persistent shuffle_id.
- Implement wraparound/looping for all slideshow sequence types. When users navigate to the end of any sequence (new, tagged, untagged, or their shuffle variants), the slideshow automatically loops back to the beginning. Forward navigation from the last photo returns to the first photo, and backward navigation from the first photo wraps to the last photo. This provides seamless infinite scrolling through all filtered photo sets.
